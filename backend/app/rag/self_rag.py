...
AsyncGenerator = Callable[[str, list[Passage], bool], Awaitable[str]]
...
class _Retriever(Protocol):
    ...
    async def atavily_result(self, query: str): ...
...
async def arun_self_rag(
    ...
    """Async version of run_self_rag: parallelizes retrieval grading."""
    max_retries = settings.max_rag_retries if max_retries is None else max_retries

    retrieval = await retriever.aretrieve(query, k=k, n=n, filters=filters)
    passages = retrieval.passages
    source: RetrievalSource = retrieval.source
    tried_tavily = source == "tavily"
    ...
    adequacy = await critics.agrade_adequacy(query, answer)
    if not adequacy.adequate and not tried_tavily:
        log.info("self_rag.adequacy_fail_escalate", query=query, missing=adequacy.missing)
        tav = await retriever.atavily_result(query)
        if tav.passages:
            passages = tav.passages
            source = "tavily"
            tried_tavily = True
            answer, grounding, more = await _agrounded_generation(
                query, passages, generate, critics, max_retries, strict_first=True
            )
            retries += more
            confidence = "medium" if grounding.grounded else "low"
...
async def _agrounded_generation(
    query: str,
    passages: list[Passage],
    generate: AsyncGenerator,
    critics: Critics,
    ...
    """Async generation loop with retry cap."""
    texts = [p.content for p in passages]
    attempt = 0
    while True:
        strict = strict_first or attempt > 0
        answer = await generate(query, passages, strict)
        grounding = await critics.agrade_grounding(answer, texts)
        if grounding.grounded or attempt >= max_retries:
            if not grounding.grounded:
                log.warning("self_rag.cap_reached", query=query, retries=attempt)
            return answer, grounding, attempt
...
def make_async_llm_generator(llm) -> AsyncGenerator:
    ...
    async def _generate(query: str, passages: list[Passage], strict: bool) -> str:
        sys = strict_sys if strict else base_sys
        ctx = "\n\n---\n\n".join(
            f"[{p.filename or p.source_path}]\n{p.content}" for p in passages
        ) or "(no passages retrieved)"
        ...
        msg = await (prompt | llm).ainvoke({"ctx": ctx, "query": query})
        return getattr(msg, "content", str(msg))
